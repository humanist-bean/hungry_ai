...
def train(dataloader, ixtoword, netG, netD, text_encoder, image_encoder,
    ...
    for epoch in tqdm(range(state_epoch + 1, cfg.TRAIN.MAX_EPOCH + 1)):
        ...
        for step in tqdm(range(len(data_iter))):
            data = next(data_iter) # Alder changed from '.next()'
...
if __name__ == "__main__":
    ...
    # Alder commented out line above and added lines below
    old_dset_size = 5450
    text_encoder = RNN_ENCODER(old_dset_size, nhidden=cfg.TEXT.EMBEDDING_DIM)
