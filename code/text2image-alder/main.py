...
from datasets import TextDataset # Alder uncommented line above and commented this line
...
def gen_sample(text_encoder, netG, device, wordtoix):
    ...
    captions = ['small and shiny black beans with a smooth texture',
                'shiny and plump black beans with a beautiful appearance',
                'plump and tender black beans with a meaty texture',
                'chunky potatoes with a soft and fluffy texture',
                'crispy potatoes with a golden and crispy exterior',
                'diced potatoes with a uniform size and shape',
                'the thickness of tomato soup can vary from a thin broth to a hearty stew',
                'the red color of tomato soup is a symbol of its warmth and comfort',
                'the thickness of tomato soup can make it a filling meal on its own',
                'the tacos are crispy and golden brown with a of bright green guacamole on the side']
    ...
    for cl, line in enumerate(captions):
        for cw, word in enumerate(line):
            #print(f"TEST: cw = {cw}, word.lower() = {word.lower()}")
    ...
    for step in range(50):
        ...
        with torch.no_grad():
            ...
            # use different noise
            noise = []
            for i in range(batch_size): # Alder changed this line
        ...
        for j in range(batch_size):
            ...
            print(f"Saving generated image to: {fullpath}") # Alder added this line
            im.save(fullpath)
            ...
            im = Image.fromarray(im)
            fullpath = '%s_%d.png' % (s_tmp, step)
            print(f"Saving generated fusion mask to: {fullpath}" ) # Alder added this line
            im.save(fullpath)


def gen_sample_alder(text_encoder, netG, device, wordtoix, caption, class_label, caption_num):
    """
    generate a sample for the given caption
    - caption should be a single string here, NOT a list of captions
    - label should be the label name for the classifier's desired classes,
    e.g. "chicken and waffles" or "tomato soup with grilled cheese"
    
    This function was made for Alder French's "HungryAI Food Generation"
    experiment, and its purpose is to help Alder generate images of "Combo Foods"
    for training a classifer network, given the caption.
    """
    # caption to idx
    # split string to word
    #for c, i in enumerate(captions):
    ...
    caps = torch.zeros((1, 18), dtype=torch.int64) # 1 because we are generating images for a single caption

  
    for cw, word in enumerate(caption):
        #print(f"TEST: cw = {cw}, word.lower() = {word.lower()}")
        try:
            caps[0][cw] = wordtoix[word.lower()]
        except KeyError:
            print(f"Encountered key error with key: {word.lower()}")
            print(f"Replacing that key with 'and' and continuing...")
            caps[0][cw] = wordtoix["and"]
            continue
        except Exception as e:
            # NOTE: put exception to catch when word is not in dictionary here
            print(f"EXCEPTION RAISED IN gen_sample_alder(): {e}")
            raise
    ...
    caps = caps.to(device)
    cap_len = [len(caption)] #len(caption)


    cap_lens = torch.tensor(cap_len, dtype=torch.int64).to(device)
    ...
    batch_size = 1 #len(caption) # Batch size should == 1
    print(f"TEST: I think batch size should equal 1 here: batch_size = {batch_size}")
    s_tmp = model_dir[:model_dir.rfind('.pth')]
    fake_img_save_dir = '%s/%s/%s' % (s_tmp, split_dir, class_label) 
    mkdir_p(fake_img_save_dir)

    images_per_caption = 10
    for step in range(caption_num * images_per_caption, (caption_num + 1) * images_per_caption): # lets set this to 4 for 4 images per caption, so we can use every caption
...
def train(dataloader, ixtoword, netG, netD, text_encoder, image_encoder,
    ...
    for epoch in tqdm(range(state_epoch + 1, cfg.TRAIN.MAX_EPOCH + 1)):
        ...
        for step in tqdm(range(len(data_iter))):
            data = next(data_iter) # Alder changed from 'data_iter.next()'
        ...
        # ALDER ADDED IF STATEMENT BELOW - Honestly can't believe I need to add this.    
...
if __name__ == "__main__":
    ...
    if cfg.B_VALIDATION:
        ...
        ixtoword = dataset.ixtoword
        print(dataset.n_words, dataset.embeddings_num)
        print(f"The length of the dataset is: {len(dataset)}") # Alder added this line
        assert dataset
        dataloader = torch.utils.data.DataLoader(
            dataset, batch_size=batch_size, drop_last=True,
            shuffle=True, num_workers=int(cfg.WORKERS))
    ...
    # Alder commented out line above and added line below
    ...
    # Alder commented out line above and made new lines below
    #old_dset_size = 5450
    #idk_why_but_58 = 58
    #text_encoder = RNN_ENCODER(idk_why_but_58, nhidden=cfg.TEXT.EMBEDDING_DIM)
    state_dict = torch.load(cfg.TEXT.DAMSM_NAME, map_location=lambda storage, loc: storage)
    text_encoder.load_state_dict(state_dict)
    text_encoder.cuda() # alder commented out this line and added one below
    #text_encoder.cpu()
    ...
    image_encoder.cuda() # alder commented out this line and added one below
    #image_encoder.cpu()
