...
def get_imgs(img_path, imsize, bbox=None,
    ...
    try:
        ...
        print("Encountered exception, most likely a corrupted image. Using randomly generated \
            tensor in place of the image.")
        return []
    ...
    if cfg.GAN.B_DCGAN:
        ...
        for i in range(cfg.TREE.BRANCH_NUM):
            # print(imsize[i])
            i = 0 # alder
            if False:# i < (cfg.TREE.BRANCH_NUM - 1):
...
class TextDataset(data.Dataset):
    ...
    def load_captions(self, data_dir, filenames):
        ...
        for i in range(len(filenames)):
            ...
            cap_path = filenames[i][:-3] + ".txt"
            print(f"TEST in 'load_captions', cap_path is: {cap_path}")
            with open(cap_path, "r") as f:
                captions = f.read()
                try:
                  captions = captions.decode('utf8')
                except AttributeError:
                    print("Encountered an attribute error in load captions")
                    continue
                captions = captions.split( '\n' )
                print(f"TEST: captions are {captions}")
                cnt = 0
                for cap in captions:
                    if len(cap) == 0:
                        continue
    ...
    def __getitem__(self, index):
        ...
        img_name = key # Alder
        print(f"The image path is {img_name}")
        try:
            ...
            print('imgs has length <= 0...')
            tensor = torch.rand(3, 256, 256)
            rand_transform = transforms.ToPILImage()
            image = rand_transform(tensor)
            imgs.append(image)
        ...
        caps, cap_len = self.get_caption(new_sent_ix) # Return last index if image is corrupted?
        print("MADE IT")
        caps = [sentence for sentence in caps if len(sentence) > 0]
        cap_len = len(caps)
