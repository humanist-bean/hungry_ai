...
- So far this script ran when I deleted the captions.pickle file and let it 
create a new one to run this pretraining script.  Note that after 550 epochs
I had a loss of about 5.5 - 6 on both the text and image encoders.
"""
...
def train( dataloader, cnn_model, nlp_model, text_encoder_type, batch_size,
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        #print(f"TEST in train(): data[4] = {data[4]}")

        imgs, captions, cap_lens, \
            class_ids, keys = prepare_data( data )
        #print(f"imgs:{imgs}, captions:{captions}, cap_lens:{cap_lens}")
        #print(f"class_ids:{class_ids}, keys:{keys}")
        ...
        if text_encoder_type == 'rnn':
            ...
            #print(f"TEST: {captions}")
            #print(f"TEST: batch size is {batch_size}")
            #hidden = hidden.transpose(0, 1) # Alder added this
...
def build_models( text_encoder_type ):
    ...
    labels = Variable(torch.LongTensor(range(batch_size))) #Alder - Variables are deprecated!
...
if __name__ == "__main__":
    ...
    print(f"In main: dataset.n_words = {dataset.n_words}, dataset.embeddings_num = {dataset.embeddings_num}")
    assert dataset
    ...
    try:
        ...
        for epoch in range(start_epoch, cfg.TRAIN.MAX_EPOCH):
            ...
            if (epoch % cfg.TRAIN.SNAPSHOT_INTERVAL == 0 or
                epoch == (cfg.TRAIN.MAX_EPOCH - 1) ): # Alder added -1.
